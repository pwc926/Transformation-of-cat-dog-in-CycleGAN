...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, direction='A2B', data_type='float32', nch=3, transform=[]):
        ...
        dataA = [f for f in os.listdir(self.data_dir_a) if f.endswith('.png')]
        dataA.sort(key=lambda f: int(''.join(filter(str.isdigit, f))))

        dataB = [f for f in os.listdir(self.data_dir_b) if f.endswith('.png')]
        dataB.sort(key=lambda f: int(''.join(filter(str.isdigit, f))))

        self.names = (dataA, dataB)

    def __getitem__(self, index):
        # Ensure index is within bounds for both domains
        max_index = min(len(self.names[0]), len(self.names[1]))
        if index >= max_index:
    ...
    def __len__(self):
        # Return the minimum length to avoid index errors
        return min(len(self.names[0]), len(self.names[1]))
...
class Rescale(object):
    ...
    def __call__(self, data):
        ...
        # If already at target size, skip resizing
...
class RandomCrop(object):
    ...
    def __call__(self, data):
        ...
        # If already at target size, skip cropping
        if (h, w) == (new_h, new_w):
            return {'dataA': dataA, 'dataB': dataB}
